...
#met['solar'] = met.net_mod * 3600 / 10**6

#per = slice('20140101','20150101')
...
plt.plot(dd.Date,dd.AirTC_Avg)

plt.show()

#plt.plot(dd.Date, dd.solar)
...
period = slice('20140317','20140813')
...
out = outputs_water_storage[0].iloc[:-58,:]
out['swc10'] = measured_met.VWC_30cm_corr.iloc[:-58].values
out['swc30'] = measured_met.VWC_30cm_corr.iloc[:-58].values
out['swc50'] = measured_met.VWC_50cm_corr.iloc[:-58].values

out['ET_mod'] = modelled_water_flux.modelled_ET.iloc[:-58]
#out['ET_measured'] = measured_met.evap_corr_mm.iloc[:-2].values
out['ET_measured'] = out['ET_mod'] = modelled_water_flux.modelled_ET.iloc[:-58]
out['ET_PM'] = dd['ReferenceET'][period].iloc[:-58].values
out['Precipitation'] = dd['Precipitation'][period].iloc[:-58].values
out['model_canopy_cover'] = modelled_crop_growth.canopy_cover.iloc[:-58].values
out['ndvi_maize'] = dd['ndvi_maize'][period].iloc[:-58].values
