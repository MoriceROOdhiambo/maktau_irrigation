...
period = slice('20180317','20180813')
...
out = outputs_water_storage[0].iloc[:-67,:]


out['swc10'] = measured_met.VWC_30cm_corr.iloc[:-67].values
out['swc30'] = measured_met.VWC_30cm_corr.iloc[:-67].values
out['swc50'] = measured_met.VWC_50cm_corr.iloc[:-67].values

out['ET_mod'] = modelled_water_flux.modelled_ET.iloc[:-67]
out['ET_measured'] = modelled_water_flux.modelled_ET.iloc[:-67]
#out['ET_measured'] = measured_met.evap_corr_mm.iloc[:-2].values
out['ET_PM'] = dd['ReferenceET'][period].iloc[:-67].values
out['Precipitation'] = dd['Precipitation'][period].iloc[:-67].values
out['ndvi_maize'] = dd['ndvi_maize'][period].iloc[:-67].values
out['model_canopy_cover'] = modelled_crop_growth.canopy_cover.iloc[:-67].values
out['ndvi_maize'] = dd['ndvi_maize'][period].iloc[:-67].values
