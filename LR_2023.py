...
period = slice('20230317','20230813')
...
out = outputs_water_storage[0].iloc[:-70,:]
out['swc10'] = measured_met.VWC_30cm_corr.iloc[:-70].values
out['swc30'] = measured_met.VWC_30cm_corr.iloc[:-70].values
out['swc50'] = measured_met.VWC_50cm_corr.iloc[:-70].values

out['ET_mod'] = modelled_water_flux.modelled_ET.iloc[:-70]
#out['ET_measured'] = measured_met.evap_corr_mm.iloc[:-2].values
out['ET_PM'] = dd['ReferenceET'][period].iloc[:-70].values
out['Precipitation'] = dd['Precipitation'][period].iloc[:-70].values



# P1: 10 cm, P2: 20 cm

out['swc10_plot'] = sm_dd.Port_1.iloc[:-70].values

out['swc20_plot'] = sm_dd.Port_2.iloc[:-70].values
...
out['model_canopy_cover'] = modelled_crop_growth.canopy_cover.iloc[:-70].values
...
ax.set_title('Rainfed 2021', fontsize=20, weight='bold')
