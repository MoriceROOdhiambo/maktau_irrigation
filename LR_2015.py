...
initWC = InitialWaterContent(wc_type='Num', method='Layer', depth_layer=[1,2,3,4,5], value=[0.07,0.07,0.07,0.07,0.07])
...
period = slice('20150317','20150813')
...
out = outputs_water_storage[0].iloc[:-78,:]
out['swc10'] = measured_met.VWC_30cm_corr.iloc[:-78].values
out['swc30'] = measured_met.VWC_30cm_corr.iloc[:-78].values
out['swc50'] = measured_met.VWC_50cm_corr.iloc[:-78].values

out['ET_mod'] = modelled_water_flux.modelled_ET.iloc[:-78]
#out['ET_measured'] = measured_met.evap_corr_mm.iloc[:-2].values
out['ET_PM'] = dd['ReferenceET'][period].iloc[:-78].values
out['Precipitation'] = dd['Precipitation'][period].iloc[:-78].values
...
out['model_canopy_cover'] = modelled_crop_growth.canopy_cover.iloc[:-78].values
out.model_canopy_cover.plot(ax=ax, fontsize=18)
#out.modis_CC_linear_maize.plot(ax=ax)
...
b = calculate_sws(swc, z)

measured_tsw = b

print(b, measured_tsw)
...
ax.set_title('Rainfed 2015', fontsize=20, weight='bold')
ax.legend(loc='best', fontsize=16)

mean = np.sum(measured_tsw)/len(measured_tsw)

c = np.mean(measured_tsw)
